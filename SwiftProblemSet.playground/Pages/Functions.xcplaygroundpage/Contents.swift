...
func beginsWithVowel(_ word: String) ->Bool{
    ...
    if(word != ""){
        ...
        return vowel.contains(word.lowercased().characters[word.startIndex])
    }
    return false
...
func funWithWords(_ word: String) ->String{
    ...
    if word != ""{
        let vowel: [Character] = ["a", "e", "i", "o", "u"]
        if(vowel.contains(word.lowercased().characters[word.startIndex])){
