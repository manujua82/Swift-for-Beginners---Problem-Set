...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if((bugs<10)  && hasMusic && (numberOfLevels>5)){
        ...
        var numIssues = 1
        var message = "\(game) is not ready for sale, it need the following: \n"
        if bugs > 10{
            message += "\t\(numIssues). fixed bugs. \n"
            numIssues += 1
        }
        if !music{
            message += "\t\(numIssues). Add music. \n"
            numIssues += 1
        }
        if levels <= 5{
            message += "\t\(numIssues). The game has \(levels) levels. It need more than 5 levels. \n"
        }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike && run {
        print("\(name) is now ready for duathlon")
    }else if bike && !run{
        print("\(name) cannot finish run")
    }else if !bike && run{
        print("\(name) cannot finish bike")
    }else if !bike && !run{
        print("\(name) cannot finish bike and run")
    }
}
